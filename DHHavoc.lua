...
Action[ACTION_CONST_DEMONHUNTER_HAVOC] = {
    ...
    -- Rotation       
    FelRush                                = Create({ Type = "Spell", ID = 344865                                               }),
    VengefulRetreat                                = Create({ Type = "Spell", ID = 344866                                               }),
    GlaiveTempest                                = Create({ Type = "Spell", ID = 342817                                               }),
...
local Temp                                     = {
    ...
    for key, val in pairs(Action[ACTION_CONST_DEMONHUNTER_HAVOC]) do 
...
local function Interrupts(unitID)
    ...
    if useKick and not notInterruptable and A.Disrupt:IsReady(unitID) then 
        return A.Disrupt
    end         
end
...
A[3] = function(icon)
    ...
    function EnemyRotation(unitID)    
        ...
        local function CDs()  
            ...
            if A.Metamorphosis:IsReady(unitID, true) then
        ...
        -- [[ Single Target ]]
        local function ST()
            if A.FelRush:IsReady(unitID, true) and Unit(unitID):GetRange() >= 18 then
                return A.FelRush:Show(icon)
            end
            if A.GlaiveTempest:IsReady(unitID, true) then
